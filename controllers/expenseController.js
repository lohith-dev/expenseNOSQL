...
async function fetchDataFromDatabase(offset, limit,id) {
    const result = await expenseModel.find({ userId: id })
                  .skip(offset)
                  .limit(limit);
...
const getappntdata = async (req,res)=>{
     try{
        ...
        const totalItemCount = await expenseModel.countDocuments({ userId: id });
...
const downloadExpense = async (req,res)=>{
   try{
    const expenses = await expenseModel.find({userId:req.user})
    console.log("lllllll",expenses);
    const stringfiedExpenses = JSON.stringify(expenses);
    const userId= req.user.id;
    const filename = `Expense${userId}/${new Date()}.txt`;
    const fileURL=await uploadToS3(stringfiedExpenses,filename);
    const data = await fileModel.create({fileUrl:fileURL,userId:req.user})
    if(data){
